...
type RightSidebarTab = "settings" | "ai" | "history";
...
export function RightSidebar() {
  ...
  const application = useQuery(
    ...
    isTailoredResume && applicationId ? { id: applicationId as Id<"applications"> } : "skip",
  );
      ...
      <TabsContent value="ai" className="mt-0 min-h-0 flex-1 data-[state=inactive]:hidden">
        <aside className="flex h-full min-h-0 w-full flex-col overflow-hidden border-l">
          <ScrollArea className="min-h-0 flex-1">
            <div className="p-4">
              <AiActionsTab application={application as any} />
            </div>
          </ScrollArea>
        </aside>
      </TabsContent>

      <TabsContent value="history" className="mt-0 min-h-0 flex-1 data-[state=inactive]:hidden">
        <aside className="flex h-full min-h-0 w-full flex-col overflow-hidden border-l">
          <ScrollArea className="min-h-0 flex-1">
            <div className="p-4">
              <HistoryPanel />
